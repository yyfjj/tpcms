;(function () {
    ...
    if (ajaxForm_list.length) {
        Wind.use('validate', 'ajaxForm', 'artDialog', function () {//2017-06-14 by benjamin 修改载入顺序
 ...
 * @param extra_params 额外参数，object
 * @param app  应用名,CMF的应用名
 * @author benjamin by 2017-06-09
 */
function upload_one_file(dialog_title, input_selector, extra_params, app) {
    open_upload_dialog(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);
        $(input_selector + '-preview').attr('src', files[0].preview_url);
        $(input_selector + '-name').val(files[0].name);
    }, extra_params, 0, 'file', app);
}
/**
 * 单个图片上传
 * @param dialog_title 上传对话框标题
