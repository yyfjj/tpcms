...
function U($url='',$vars='',$suffix=true,$domain=false) {
    ...
    if(C('URL_MODEL') == 0) { // 普通模式URL转换
        $url        =   __APP__.'?'.C('VAR_MODULE')."={$module}&".http_build_query_from_array(array_reverse($var));
        if($urlCase){
            $url    =   strtolower($url);
        }        
        if(!empty($vars)) {
            $vars   =   http_build_query_from_array($vars);
            $url   .=   '&'.$vars;
        }
...
* @author benjamin
*/
function http_build_query_from_array($arr)
{
	$q = '';
	$pieces=array();
	foreach ($arr as $k => $v)
