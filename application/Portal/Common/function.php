...
* @author benjamin
*/
function sp_get_nav_level_one($id)
{
	#第一步，取得path路径
	$pathArr = _sp_get_nav_path($id);
	...
	//当是0-18-62-70
	//当是0-18-62-70-72
	if(count($pathArr) == 2 || count($pathArr) == 3 || count($pathArr) == 4 || count($pathArr) == 5){
		$navArr =  M("nav")->where("status=1 and parentid=".$pathArr['0'])->order("listorder asc")->select();
		foreach ($navArr as $k => $v){
			if($v['id'] == $pathArr[1]){
				$navArr[$k]['selected'] = true;
			}else {
				$navArr[$k]['selected'] = false;
			}
		}
	}
	
	return _sp_rebuild_nav_href($navArr);
}


function sp_get_nav_level_two($nav_id)
{
	#第一步，取得path路径
	$pathArr = _sp_get_nav_path($nav_id);
	
	#第二步，根据path路径判断
	//当是0-18
	if(count($pathArr) == 2){
		$navArr = M("nav")->where("status=1 and parentid=".$pathArr['1'])->order("listorder asc")->select();
		foreach ($navArr as $k => $v){
			if($k === 0){
				$navArr[$k]['selected'] = true;
			}else{
	...
	//当是0-18-62-70
	//当是0-18-62-70-72
	if(count($pathArr) == 3 || count($pathArr) == 4 || count($pathArr) == 5){
		$navArr =  M("nav")->where("status=1 and parentid=".$pathArr['1'])->order("listorder asc")->select();
		foreach ($navArr as $k => $v){
			if($v['id'] == $pathArr[2]){
				$navArr[$k]['selected'] = true;
...
function sp_get_nav_level_three($nav_id)
{
	#第一步，取得path路径
	$pathArr = _sp_get_nav_path($nav_id);
	
	#第二步，根据path路径判断
	//当是0-18(修复为下一个层级,通过增加pathArr[2]方式修复，比如修复为0-18-62)
	if(count($pathArr) == 2){
		$navArr = sp_get_nav_level_two($nav_id);
		foreach ($navArr as $k => $v){
			if($v['selected'] === true){
	...
	//当是0-18-62
	if(count($pathArr) == 3){
		$navArr = M("nav")->where("status=1 and parentid=".$pathArr['2'])->order("listorder asc")->select();
		foreach ($navArr as $k => $v){
			if($k === 0){
				$navArr[$k]['selected'] = true;
			}else{
	...
	//当是0-18-62-70
	//当是0-18-62-70-72
	if(count($pathArr) == 4 || count($pathArr) == 5){
		$navArr =  M("nav")->where("status=1 and parentid=".$pathArr['2'])->order("listorder asc")->select();
		foreach ($navArr as $k => $v){
			if($v['id'] == $pathArr[3]){
				$navArr[$k]['selected'] = true;
...
function sp_get_nav_level_four($nav_id)
{
	#第一步，取得path路径
	$pathArr = _sp_get_nav_path($nav_id);
	...
	//当是0-18
	//当是0-18-62
	//(修复为下一个层级,通过增加pathArr[3]方式修复，比如修复为0-18-62-70)
	if(count($pathArr) == 2 || count($pathArr) == 3){
		$navArr = sp_get_nav_level_three($nav_id);
		foreach ($navArr as $k => $v){
			if($v['selected'] === true){
				$pathArr = explode("-", $v['path']);
			}
	...
	//当是0-18-62-70 默认排第一位的出于选中状态
	if(count($pathArr) == 4){
		$navArr = M("nav")->where("status=1 and parentid=".$pathArr['3'])->order("listorder asc")->select();
		foreach ($navArr as $k => $v){
			if($k === 0){
				$navArr[$k]['selected'] = true;
			}else{
	...
	//当是0-18-62-70//默认全部出于选中状态
	if(count($pathArr) == 4){
		$navArr = M("nav")->where("status=1 and parentid=".$pathArr['3'])->order("listorder asc")->select();
		
		$navThreeArr = sp_get_nav_level_three($nav_id);
		foreach ($navThreeArr as $key => $value){
			if($value['selected'] === true){
				$value['selected'] = true;
				$value['label']   = '全部';
				array_unshift($navArr,$value);
			}
		}
...
/**
* @desc 
* @param int $nav_id nav表自增id
* @return 
* @example 
* @date 2017年5月11日
* @author benjamin
*/
function _sp_get_nav_path($nav_id)
{
	$where['id']     = $nav_id;
	$where['status'] = 1;
	$navArr = M("nav")->where($where)->find();
	$pathArr = explode("-", $navArr['path']);
	return $pathArr;
}
...
* @author benjamin
*/
function _sp_rebuild_nav_href($navArr)
{
	foreach ($navArr as $k => $v){
		$tmp = sp_get_menu_info($v['id']);
		$href = $tmp['href']."&nav_id={$v['id']}";
		$navArr[$k]['href'] = $href;
	}
	return $navArr;
}