 ...
 * @author huajie <banhuajie@163.com>
 */
class AttributeController extends ModuleBaseController{
...
//     	dump($_REQUEST);
        ...
        if(!$res){
        	...
        	$modelName = M("model")->where("id=".intval($_REQUEST['model_id']))->getField("name");
        	$where['model'] = $modelName;
        	$where['app']   = 'Module';
        	if(M("menu")->where($where)->count()){
        		
        	}else{
        		$this->addMenu($modelName);
        	}
